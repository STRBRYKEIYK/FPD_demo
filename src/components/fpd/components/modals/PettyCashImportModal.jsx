...
import { generatePettyCashImportTemplate, parsePettyCashImportFile } from '../../../../utils/imports/PettyCashImportTemplate';

/**
 * Modern Import Modal for Petty Cash Vouchers
 * Features a refined drag-and-drop zone and clean status reporting.
 */
export default function PettyCashImportModal({ isOpen, onClose, onImport, isDarkMode }) {
  // --- STATE ---
  ...
  const handleDownloadTemplate = async () => {
    try {
      await generatePettyCashImportTemplate();
    } catch (error) {
  ...
  const handleFile = async (file) => {
    ...
    if (file.name !== 'import_petty_cash.xlsx') {
      setParseErrors([{ 
        type: 'error', 
        message: `Invalid file name. Expected "import_petty_cash.xlsx" but got "${file.name}"` 
      }]);
      return;
    }
    ...
    try {
      const { vouchers, errors } = await parsePettyCashImportFile(file);
  ...
  const handleImport = async () => {
    ...
    setIsProcessing(true);
    try {
      const { vouchers } = await parsePettyCashImportFile(selectedFile);
      ...
      <div className={`relative rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] ${bgColor}`}>
        ...
        <div className={`px-6 py-5 border-b flex justify-between items-center ${borderColor}`}>
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className={`text-xl font-bold ${textColor}`}>Import Petty Cash Vouchers</h3>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
          ...
          <div className={`mb-6 p-4 rounded-xl border ${isDarkMode ? 'bg-neutral-900/40 border-neutral-700/50' : 'bg-neutral-100/50 border-neutral-200'}`}>
            ...
            <ul className={`space-y-2 text-xs ${isDarkMode ? 'text-neutral-200/80' : 'text-neutral-900/70'}`}>
                ...
                <li className="flex items-start gap-2">
                  ...
                  <span>Name file exactly <strong>import_petty_cash.xlsx</strong>.</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="font-bold opacity-50">4.</span> 
                  <span>Fill <strong>amount</strong> and select valid <strong>VAT Type</strong>.</span>
                </li>
            </ul>
          </div>
          ...
          >
            ...
            <span className="font-medium text-sm">Download Template (import_petty_cash.xlsx)</span>
          </button>
          ...
          {!selectedFile ? (
            ...
            <div
              ...
              className={`relative group border-2 border-dashed rounded-2xl p-10 text-center transition-all cursor-pointer ${
                dragActive 
                  ? 'border-neutral-500 bg-neutral-100/50 dark:bg-neutral-900/20' 
                  : `${borderColor} hover:border-neutral-400 hover:bg-gray-50 dark:hover:bg-gray-800/50`
              }`}
            >
              ...
              <div className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-4 transition-colors ${
                dragActive ? 'bg-neutral-200 text-neutral-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 group-hover:text-neutral-500'
              }`}>
                <Upload size={28} />
              </div>
